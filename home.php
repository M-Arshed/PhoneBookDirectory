...
<?php
$query ="select * from contact where email = '$uid' OR emailfk = '$uid' " ;
...
while($row = mysqli_fetch_array($result))
  ...
  if($row['emailfk'] != ''){
  echo "<td>" . $row['emailfk'] . "</td>";
}
else{
  echo "<td> Added By Admin</td>";
}
...
?>
  ...
  <td><a href="share.php?sharecontact=<?php echo $row['contactid']; ?>"><button class="btn btn-primary">Share</button></a></td>
                
    <?php    
}else{
  echo "<td>Cant Update</button></td>";
  echo "<td> Cant Delete</td>";
  echo "<td> Cant Share</td>";
}
